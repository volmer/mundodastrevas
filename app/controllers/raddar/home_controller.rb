module Raddar
  class HomeController < Raddar::ApplicationController
    ...
    def index
      featured_zines = Zines::Zine.with_posts.where(starred: true).order('last_post_at DESC').limit(4)

      @featured_posts = featured_zines.map {|zine| zine.posts.order('created_at DESC').first }

      @recent_posts = Zines::Zine.with_posts.where.not(id: featured_zines.map(&:id)).order('last_post_at DESC').limit(5).map {|zine| zine.posts.order('created_at DESC').first }
