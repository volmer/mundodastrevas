module Raddar
  module Zines
    class ZinesController < Zines::ApplicationController
      ...
      def index
        ...
        if params[:user_id]
          ...
          @zines = @user.zines.order('COALESCE(last_post_at, created_at) DESC').page(params[:page])

          render 'user_index'
        else
          @zines = Zine.with_posts.order('last_post_at DESC').page(params[:page])
      ...
      def create
        ...
        if @zine.save
          redirect_to @zine, notice: t('flash.zines.zines.create')
        else
          render action: 'new'
        end
      end
      ...
      def update
        if @zine.update(zine_params)
          redirect_to @zine, notice: t('flash.zines.zines.update')
        else
          render action: 'edit'
        end
      end
      ...
      def destroy
        ...
        redirect_to zines_path, notice: t('flash.zines.zines.destroy')
      end
