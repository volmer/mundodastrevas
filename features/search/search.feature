...
Feature: Search
  ...
  Scenario: Query a term through users and pages
    ...
    When I fill in "query" with "stark"
  ...
  Scenario: Search for inexistent data
    ...
    When I fill in "query" with "arya"
    And I press enter in "query"
    Then I see the info message "Nenhum resultado encontrado."

  Scenario: Find a term in a page content
    Given there is a page with the following data:
      | title           | content           |
      | Valar Morghulis | All men must die. |
    When I fill in "query" with "all men must die"
    And I press enter in "query"
